...
content = {
    True: [
        ContentUnit("text", "- Вот твоя футболка. Важно! В 18:00 в аудитории 107 в университете состоится организационная встреча. На ней быть обязательно! Уже послее нее можно будет сходить на ужин. На обороте твоего бейджа есть штрих-код, по нему и пробьешь питание в столовой."),
...
content = {
    True: [
        ...
        ContentUnit("text", "Убрав за собой поднос, вы с соседом направились в читальный зал на 3-м этаже на ориентационный семинар, который в рамках Летней школы будет проводиться каждый учебный день перед лекцией.")
    ],
    False: [
        ContentUnit("text", "- Да? Как же? – поинтересовалась ты."),
        ContentUnit("text", "- В личном кабинете, очень удобная штука. Можешь найти по этой ссылке."),
        ContentUnit("text", "https://my.university.innopolis.ru/"),
        ContentUnit("text", "Убрав за собой поднос, вы с соседкой направились в читальный зал на 3-м этаже на ориентационный семинар, который в рамках Летней школы будет проводиться каждый учебный день перед лекцией.")
    ]
}
...
content = [
    ...
    ContentUnit("text","На семинаре вы ближе познакомились с другими участниками BootCamp и определили тему дня, чтобы уже вечером в маленьких группах с тьютором обработать полученный материал.", delay=3),
...
# III_a_00_00
content = [ContentUnit("text", "Еще один день в BootCamp, который, как всегда, начинается с  ориентационного семинара, за которым следует череда занятий."),
...
content = [
    ...
    ContentUnit("text", "- столовая 4 корпуса 'INNOEDA!' (@GeekCaffeeBot)"),
 	ContentUnit("text", "- пиццерия 'Cacio e Vino' (https://goo.gl/NeHeCT, доставки нет, но можно делать предзаказ на вынос)"),
    ContentUnit("text",
