...
buttons = {"Дальше": "I_a_01_00",
           ...
           "НАЧАТЬ ПЯТЫЙ ДЕНЬ":"V_a_00_00",
           "НАЧАТЬ ШЕСТОЙ ДЕНЬ":"VI_a_00_00"}
...
content = [
    ...
    ContentUnit("text",
                "Ты замечаешь, что одна из ваших подруг сидит в толстовке с надписью “Innopolis University” (тут можно вставить фото, либо добровольца в толстовке, либо просто любое фото толстовки)"),
...
content = {
    True: [
        ContentUnit("text", "Вот и наступил последний день в BootCamp"),
...
content = [
    ...
    ContentUnit("text", "-И когда она будет? И что это вообще такое?"),
    ContentUnit("text", "-Точную информацию нам должны сказать к концу года, думаю. Поэтому не стоит пока делать точные планы, хотя бы начало лета постараться оставить свободным. Я знаю, что они подразделяются на административные и научные, и в компании мы можем идти только после второго курса."),
...
# VI_b_02_01
content = [
    ContentUnit("text", "При административной делают разные проекты для университета. А научные для того чтобы прокачаться в какой-то определенной области, например, если тебе нравится робототехника, можно пойти на стажировку в лабораторию к профессору Мавридису. Мне студенты рассказали."),
...
content = [
    ...
    ContentUnit("text","В читальном зале собралось много людей: некоторые в национальной одежде, кто-то принес национальные угощения."),
