...
buttons = {"Дальше": "I_a_01_00",
           "НАЧАТЬ ТРЕТИЙ ДЕНЬ":"III_a_00_00",
           "НАЧАТЬ ЧЕТВЕРТЫЙ ДЕНЬ":"IV_a_00_00"}
...
states["III_c_02_01"] = State(content, buttons, callback=end)
...
content = [
    ...
    ContentUnit("text", "-Ок, буду иметь в виду. Спасибо."),
    ContentUnit("text",
...
buttons = {
    "А можно выбрать?? Либо 5 дней в неделю, либо 7?": "IV_b_01_00",
...
# IV_b_02_00
content = [
    ContentUnit("text", "- Есть такая штука, как мудл: moodle.university.innopolis.ru  Там будут лежать ответы на все твои вопросы, не переживай.")
]
buttons = {
    "Дальше":"IV_c_00_00"
}
...
# IV_c_00_00
content = [
    ContentUnit("text","Вечером все собрались у вас в комнате, поболтать и поесть пиццу. Оказывается, тут есть аж 2 места, где можно ее заказать."),
    ContentUnit("text", "Ты замечаешь, что одна из ваших подруг сидит в толстовке с надписью “Innopolis University” (тут можно вставить фото, либо добровольца в толстовке, либо просто любое фото толстовки)"),
...
#IV_c_01_00
content = [
    ContentUnit("text", "-Что достала? Ах, ты про толстовку. Ну я просто помогла пару раз отделу поддержки и развития студентов, поволонтерила на кое-каких мероприятиях, ну и, конечно, ходила на “Activities”.  И voilà – уже накопила Иннопоинты на толстовочку."),
    ContentUnit("text", "-И сколько иннопоинтов стоит? И откуда ты узнала, что тебе их хватает?"),
    ContentUnit("text", "-Ну ты реально вообще ничего не читаешь, что приходит нам, в рассылках (рассмеялась она). Эта, по-моему, 1800 иннопоинтов, а посмотреть сколько их у тебя сейчас можно на «юисе» uis.university.innopolis.ru/innopoints/products"),
    ContentUnit("text", "-Впервые слышу…"),
    ContentUnit("text", "В диалог вмешался roommate:"),
    ContentUnit("text", "-Вы знали, что иннопоинты студенты придумали и разработали эту идею вместе с отделом по поддержке и развитию студентов. Вот так вот. Если бы тогда не возникла у них такая идея, не было бы сейчас иннопоинтов. Минутка истории от меня."),
...
# IV_c_01_01
content = [
    ContentUnit("text", "-Ахаха! Нет! Я работала, не покладая рук, волонтерила и все такое. Вот и накопила себе на толстовку."),
    ...
    ContentUnit("text", "- На «юисе» uis.university.innopolis.ru/innopoints/products   Я там же и заявку на получение иннопоинтов подавала. Вообще на самом «юисе» uis.university.innopolis.ru много информации есть, там и новости пишут, например, про музыкальную комнату. "
...
buttons = {"Начать с начала":"I_a_00_00"}
